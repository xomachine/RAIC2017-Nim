...
template pointAttractiveField*(p, attractor: GridPoint): Intensity =
  ...
  Intensity(Intensity.high.float*distance/(1+1.5*distance+distance.sqrt))
...
template attackField(p, enemy: GridPoint, eff: float): Intensity =
  ...
  const safeln = safedst/(10*sqr(maxsize))
  ...
  if eff >= 0:
    ...
    Intensity(Intensity.high.float*distance/(10*sqr(maxsize)))
